...
class AppContext {
    ...
    private void doResourse() {
        // map에 저장된 객체의 iv중에 @Resourse가 붙어 있으면
        // map에서 iv의 타입에 맞는 객체를 찾아서 연결(객체의 주소를 iv 저장)
        try {
            for (Object bean : map.values()){
                for(Field fld : bean.getClass().getDeclaredFields()){
                    if(fld.getAnnotation(Resource.class)!=null){ //byName
                        fld.set(bean, getBeen(fld.getName())); //car.engine = obj;
                    }
                }
            }
            ...
            for (Object bean : map.values()){
                for(Field fld : bean.getClass().getDeclaredFields()){
                    if(fld.getAnnotation(Autowired.class)!=null){
                        fld.set(bean, getBeen(fld.getType())); //car.engine = obj;
                    }
                }
            }
        } catch (IllegalAccessException e) {
...
public class Main4 {
    public static void main(String[] args) throws Exception {
        ...
        Door door = (Door) ac.getBeen(Door.class);
